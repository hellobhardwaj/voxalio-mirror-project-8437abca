  ...
  useEffect(() => {
    ...
    const handleMouseMove = event => {
      ...
      if (isHex) {
        ...
        col = Math.round((mouseX - oX) / hexHoriz);
        const rowOffset = (col + colShift) % 2 !== 0 ? hexVert / 2 : 0;
        row = Math.round((mouseY - oY - rowOffset) / hexVert);
      } else if (isTri) {
        const halfW = squareSize / 2;
        const oX = ((gridOffset.current.x % halfW) + halfW) % halfW;
        const oY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        col = Math.round((mouseX - oX) / halfW);
        row = Math.floor((mouseY - oY) / squareSize);
      } else {
        const oX = ((gridOffset.current.x % squareSize) + squareSize) % squareSize;
        const oY = ((gridOffset.current.y % squareSize) + squareSize) % squareSize;
        col = shape === 'circle' ? Math.round((mouseX - oX) / squareSize) : Math.floor((mouseX - oX) / squareSize);
        row = shape === 'circle' ? Math.round((mouseY - oY) / squareSize) : Math.floor((mouseY - oY) / squareSize);
      }
